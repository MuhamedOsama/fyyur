...
def venues():
  ...
  for g in city_state:
    ...
    venues = db.session.query(Venue).filter((Venue.state == g[0]) & (Venue.city == g[1])).all()
    venuesData = []
    for v in venues:
      NumOfUpcomingShows = db.session.query(Show).filter((Show.start_time > currentdate) & (Show.venue_id == v.id)).count()
      venueData = {
        'id': v.id,
        'name' : v.name,
